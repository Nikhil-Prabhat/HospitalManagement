...
public interface HospitalServiceClient extends Constants {
	...
	public PatientDTO getPatientById(@PathVariable(name = ID) UUID idOfPatient);

	@GetMapping(GET_ALL_TREATMENT_HISTORY_FOR_A_PATIENT)
	public List<TreatmentHistoryDTO> getAllTreatmentHistoriesByPatientId(@PathVariable(name = ID) UUID idOfPatient);

	@GetMapping(GET_BILL_BY_PATIENT_ID)
	public BillDTO getBillByPatientId(@PathVariable(name = ID) UUID idOfPatient);
}
